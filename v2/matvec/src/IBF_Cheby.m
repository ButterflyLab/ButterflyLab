function [Factor,Rcomp] = IBF_Cheby(fun_org,xx,kk,NG,tol,tag)
...
for itx = 1:prod(npx)
    ...
    xgridcell{itx} = BF_grid(x,npx,grid,xbox);
end
for itk = 1:prod(npk)
    k = BF_idx2vec(npk,itk);
    kgridcell{itk} = BF_grid(k,npk,grid,kbox);
end
...
for ell = 1:levels
    ...
    for it_child = 1:2^Dim
        [xgrid,~] = BF_grid(BF_idx2vec(2*ones(1,Dim), ...
            it_child),npxx,grid,xbox);
        [~,xparLgrid] = BF_grid(ones(1,Dim),npxx/2,grid,xbox);
    ...
    for itx = 1:prod(npxx)
        x = BF_idx2vec(npxx,itx);
        [xgrid,~] = BF_grid(x,npxx,grid,xbox);
        [xpargrid,~] = BF_grid(floor((x-1)/2)+1,npxx/2,grid,xbox);
...
for itx = 1:prod(npxx)
    x = BF_idx2vec(npxx,itx);
    [xgrid,xLgrid] = BF_grid(x,npxx,grid,xbox);
...
for ell = 1:levels
    ...
    LagrangeMatCell = cell(2^Dim);
    for it_child = 1:2^Dim
        [kgrid,~] = BF_grid(BF_idx2vec(2*ones(1,Dim),it_child),...
            npkk,grid,kbox);
        [~,kparLgrid] = BF_grid(ones(1,Dim),npkk/2,grid,kbox);
    ...
    for itk = 1:prod(npkk)
        k = BF_idx2vec(npkk,itk);
        kgrid = BF_grid(k,npkk,grid,kbox);
        [kpargrid,~] = BF_grid(floor((k-1)/2)+1,npkk/2,grid,kbox);
...
for itk = 1:prod(npkk)
    k = BF_idx2vec(npkk,itk);
    [kgrid,kLgrid] = BF_grid(k,npkk,grid,kbox);
