function Factor = RSBFslow(fun, xx, xbox, kk, kbox, mR, tol, disp_flag)
...
% larger than max(8,mR)
levels = max(0,min(floor(log2([npx npk])-max(3,ceil(log2(mR)))))); 
LS = 2*mR^2*npk*npx;
...
for x=1:npx
    ...
    for k=1:npk
        ik = kidx{k};
        ix = xidx{x};
        [U{k},Stmp,~,Ridx{x,k},Cidx{x,k},rs{x,k},cs{x,k}] = BF_lowrank(xx(ix), kk(ik), fun, tol, tR, mR);
        P{x,k} = 1./diag(Stmp);
        U{k} = U{k}*Stmp;
    end
    xxsub = xx(xidx{x},:);
    ...
    [WPreSpr,CPreSpr] = BF_compression1D(WPreSpr,CPreSpr,U,xxsub,xidx{x},xsubbox,mR,tol,1,levels);
end
...
for k=1:npk
    ...
    for x=1:npx
        ik = kidx{k};
        ix = xidx{x};
        [~,Stmp,V{x}] = BF_lowrankidx(xx(ix),kk(ik),fun,tol,mR,Ridx{x,k},Cidx{x,k},rs{x,k},cs{x,k});
        V{x} = V{x}*Stmp;
    end
    kksub = kk(kidx{k});
    kos = kbox(1);
    ...
    [WPreSpr,CPreSpr] = BF_compression1D(WPreSpr,CPreSpr,V,kksub,kidx{k},ksubbox,mR,tol,1,levels);
end
