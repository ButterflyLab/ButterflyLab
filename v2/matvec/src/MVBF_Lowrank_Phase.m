...
disPos1m = [gd1',setdiff([disPos1-1,disPos1-2],[-1,0])];
disPos2m = [gd2',setdiff([disPos2-1,disPos2-2],[-1,0])];

if(tR<Np && tR<Nx)
    %rows
    rs = BF_RandSample(Nx,tR);
    rs = [disPos1,disPos1+1,disPos1+2,rs,disPos1m]; rs = unique(rs); rs = sort(rs);
    ...
    %columns
    cs = BF_RandSample(Np,tR);
    cs = [disPos2,disPos2+1,disPos2+2,cs,disPos2m]; cs = unique(cs); cs = sort(cs);
    ...
    %rows again
    rs = BF_RandSample(Nx,tR);
    rs = unique([[disPos1,disPos1+1,disPos1+2,rs,disPos1m] Ridx]);
    disPosSub1 = BF_find_entry(rs,disPos1);
    tmp = zeros(Nx,numel(rs));
    tmp(rs+((1:numel(rs))-1)*Nx) = 1;
    data = funt(tmp);
    ...
    %columns
    cs = BF_RandSample(Np,tR);
    cs = unique([[disPos2,disPos2+1,disPos2+2,cs,disPos2m] Cidx]);
    disPosSub2 = BF_find_entry(cs,disPos2);
    tmp = zeros(Np,numel(cs));
    tmp(cs+((1:numel(cs))-1)*Np) = 1;
    data = fun(tmp);
...
%rows
Ridx = [disPos1,disPos1+1,disPos1+2, Ridx,disPos1m]; Ridx = unique(Ridx);
...
%columns
Cidx = [disPos2,disPos2+1,disPos2+2, Cidx,disPos2m]; Cidx = unique(Cidx);
