function [Mcell,GTolcell,Ucell,HTolcell,Vcell] = ...
    BF_inComp(levels,npx,npk,tol,Mcell,GTolcell,Ucell,HTolcell,Vcell)
% This code executes the sweeping matrix compression from outside towards
% inside.
%
...
for itx = 1:prod(npxx)
    for itk = 1:prod(npkk)
        [U,S,V] = BF_svdtrunc_tol(Ucell{itx,itk},tol);
        Ucell{itx,itk} = U;
        Lcell{itx,itk} = S*V';
    end
end
...
for ell = levels:-1:1
    ...
    for itx = 1:prod(npxx)
        for itk = 1:prod(npkk)
            for it_child = 1:2^Dim
                itk_child = BF_vec2idx(npkk_child,(BF_idx2vec(npkk,itk)-1)*2 ...
    ...
    for itx_par = 1:prod(npxx_par)
        for itk_child = 1:prod(npkk_child)
            ...
            [U,S,V] = BF_svdtrunc_tol(tmpMat,tol);
...
for itk = 1:prod(npkk)
    for itx = 1:prod(npxx)
        [U,S,V] = BF_svdtrunc_tol(Vcell{itk,itx},tol);
        Vcell{itk,itx} = V';
        Rcell{itk,itx} = U*S;
    end
end
...
for ell = levels:-1:1
    ...
    for itk = 1:prod(npkk)
        for itx = 1:prod(npxx)
            for it_child = 1:2^Dim
                itx_child = BF_vec2idx(npxx_child,(BF_idx2vec(npxx,itx)-1)*2 ...
                    +BF_idx2vec(2*ones(1,Dim),it_child));
