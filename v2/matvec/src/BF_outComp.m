function [Mcell,GTolcell,Ucell,HTolcell,Vcell] = ...
    BF_outComp(levels,npx,npk,tol,Mcell,GTolcell,Ucell,HTolcell,Vcell)
% This code executes the sweeping matrix compression from inside towards
% outside.
%
...
Rcell = cell(prod(npk),prod(npx));

for itx = 1:prod(npx)
    for itk = 1:prod(npk)
        [U,S,V] = BF_svdtrunc_tol(Mcell{itx,itk},tol);
...
for ell = 1:levels
    ...
    for itx = 1:prod(npxx)
        itx_par = BF_vec2idx(npxx_par,floor((BF_idx2vec(npxx,itx)+1)/2));
...
for ell = 1:levels
    ...
    for itk = 1:prod(npkk)
        itk_par = BF_vec2idx(npkk_par,floor((BF_idx2vec(npkk,itk)+1)/2));
