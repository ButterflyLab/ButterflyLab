function [WPreSpr, CPreSpr] = BF_compression1D(WPreSpr, CPreSpr, W, kk, kkgid, kbox, r, tol, level, NL)
...
if( H <= 2*r || level > NL )
    ...
    [WPreSpr,CPreSpr] = BF_compression1D(WPreSpr, CPreSpr, W, kk, kkgid, kbox, r, tol, level+1, NL);
    return;
end
...
for k=1:2:km
    ...
    for i=1:2
        ...
        [Wtmp,S,V] = BF_svdtrunc_rank(chunk,r,tol);
        WW{i}{(k+1)/2} = Wtmp*S;
        CS{i,k} = V(1:r1,:)';
        CS{i,k+1} = V(r1+(1:r2),:)';
    end
...
for z=1:2
    ...
    [WPreSpr,CPreSpr] = BF_compression1D(WPreSpr, CPreSpr, WW{z}, kk(Id), kkgid(Id), ksubbox, r, tol, level+1, NL);
end
