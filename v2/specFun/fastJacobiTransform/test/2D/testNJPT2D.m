...
for ii=1:es
    ...
    dirfun = directjac2d(nts,ts1,ts2,wghts1,wghts2,n1,da,db,c,'FULL');
    tic;
    result3 = dirfun(c);
...
%    ncol = size(c,2);
    ...
    tic
    for i = 1:num
    [fun,rank1(ii),rank2(ii)] = NJPT2D1(nts,ts1,ts2,da,db,tR,mR,tol,1,1);
    end
    timefac(ii)=toc/num;
    %P = U*V.';
    %P = [vals P];
...
%    errornyu=norm(result1(n1)-result3)/norm(result3);
    ...
    fprintf('\n  %-5d %-9d  %-9d  %-1.6E   %-1.6E   %-1.6E   %-1.6E\n',m,rank1(ii),rank2(ii),errorour(ii),timeour(ii),timedir(ii),timefac(ii));
...
%    saveas(bf,'image13a.jpg');
    ...
    h(1) = plot(vd,3*vd-3*vd(1)+ag,'-r','LineWidth',2)
    h(2) = plot(vd,2*vd+2*log2(vd)-2*vd(1)-2*log2(vd(1))+ag,'--b','LineWidth',2);
    h(3) = plot(vd,2*vd+log2(vd)-2*vd(1)-log2(vd(1))+ag,'--k','LineWidth',2);
    ...
    legend('n^3','n^2 log^2 n','n^2 log n','n log n','n','time dir','time app','time fac','Location','NorthWest');
