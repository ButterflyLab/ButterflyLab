...
%str11='error_cheb';
fprintf('\n');
fprintf('start 3D uniform Jacobi polynomial transform stability test:');
fprintf('\n');
...
fprintf('%-6s%-11s%-11s%-11s%-15s%-15s%-15s\n',str1,str2,str3,str5,str7,str4,str10);
...
%     result3 = directjac3d(nts,ts1,ts2,ts3,wghts,wghts,wghts,n1,da,db,d);
...
%    ncol = size(c,2);
    ...
    tic
    for i = 1:num
    [fun,rank1(ii),rank2(ii),rank3(ii)] = invJPT3D1(nts,da,db,tR,mR,tol,1,1);
    end
    timefac(ii)=toc/num;
    tic;
    for j=1:num
        result = fun(reshape(result2,nts,nts,nts));
    end
    timeour(ii)=toc/num;
%    norm(result2)
%%%%%%%%%%%%%%%%%%%%%%%Greengard%%%%%%%%%%%%%%%%%
...
%    errornyu=norm(result1(n1)-result3)/norm(result3);
    ...
    fprintf('\n  %-5d %-9d  %-9d  %-9d  %-1.6E   %-1.6E   %-1.6E\n',nts,rank1(ii),rank2(ii),rank3(ii),errorour(ii),timeour(ii),timefac(ii));
