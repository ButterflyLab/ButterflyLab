function [fun,rank1,rank2,rank3] = NJPT3D1(nts,ts1,ts2,ts3,da,db,tR,mR,tol,opt,R_or_N)
%% INPUTS:
%
%  nts    - the number of the specific smaples ts1(ts2,ts3)
%  ts1    - the first specific samples given by users
%  ts2    - the second specific samples given by users
%  ts3    - the third specific samples given by users
...
%% OUTPUTS:
%
%  fun    -  a function handle computing the 3D forward nonuniform Jacobi polynomial transform such that
%                 fun(c(:)) = kron(kron(J1,J2),J3)*c(:), 
...
%                   J3(j,k) = M^(da,db)_(ts3(j),k-1)*cos(psi^(da,db)_(ts3(j),k-1)), 1=<j,k<=nts;
%  rank1  -  the rank of the lowrank approximation of the matrix
%            corresponding to J1 
%  rank2  -  the rank of the lowrank approximation of the matrix
%            corresponding to J2
%  rank3  -  the rank of the lowrank approximation of the matrix
%            corresponding to J3
%
