function  y = HSSBF_apply(Factor,p)
% HSSBF_apply applies a HSSBF matrix stored in a data-sparse HSSBF format
% in Factor to a give vector p.
%
% Input:
% p: a given vector
...
if Factor.isLeaf
    if isempty(Factor.A12) == 1
        ...
        y(1:M1,:) = Factor.A11*p(1:M1,:)+Factor.A12*p((1+M1):end,:);
    end
    if isempty(Factor.A21) == 1
        y((1+M1):end,:) = Factor.A22*p((1+M1):end,:);
    else
        y((1+M1):end,:) = Factor.A21*p(1:M1,:)+Factor.A22*p((1+M1):end,:);
    end
else
    if isempty(Factor.A12) == 1
        y(1:M1,:) = HSSBF_apply(Factor.A11,p(1:M1,:));
    else
        y(1:M1,:) = HSSBF_apply(Factor.A11,p(1:M1,:)) + BF_apply(Factor.A12,p((M1+1):end,:));
    end
    if isempty(Factor.A21) == 1
        y((1+M1):end,:) = HSSBF_apply(Factor.A22,p((1+M1):end,:));
    else
        y((1+M1):end,:) = HSSBF_apply(Factor.A22,p((1+M1):end,:)) + BF_apply(Factor.A21,p(1:M1,:));
    end
end