function  y = HSSBF_trans_apply(Factor,p)
% HSSBF_apply applies the transpose of HSSBF matrix stored in a data-sparse 
...
if Factor.isLeaf
    ...
    if isempty(Factor.A21) == 1
        y(1:M1,:) = HSSBF_trans_apply(Factor.A11,p(1:M1,:));
    else
        y(1:M1,:) = HSSBF_trans_apply(Factor.A11,p(1:M1,:)) + BF_trans_apply(Factor.A21,p((M1+1):end,:));
    end
    if isempty(Factor.A12) == 1
        y((1+M1):end,:) = HSSBF_trans_apply(Factor.A22,p((1+M1):end,:));
    else
        y((1+M1):end,:) = HSSBF_trans_apply(Factor.A22,p((1+M1):end,:)) + BF_trans_apply(Factor.A12,p(1:M1,:));
    end
end