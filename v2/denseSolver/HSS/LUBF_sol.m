function y = LUBF_sol(Factor, p,L_or_U)
% LUBF_sol applies the data-sparse inverse of a triangular HSSBF matrix to
% a vector p
%
% Input:
...
switch L_or_U
    case 'L'
        if Factor.isLeaf
            ...
            x2 = Factor.A22\(p((n1+1):end,:)-Factor.A21*x1);
            y = [x1;x2];
        else
            x1 = LUBF_sol(Factor.A11,p(1:n1,:),L_or_U);
            x2 = LUBF_sol(Factor.A22,p((n1+1):end,:)-BF_apply(Factor.A21,x1),L_or_U);
            y = [x1;x2];
        end
    case 'U'
        if Factor.isLeaf
            x2 = Factor.A22\p(n1+1:end,:);
            x1 = Factor.A11\(p(1:n1,:)-Factor.A12*x2);
            y = [x1;x2];
        else
            x2 = LUBF_sol(Factor.A22,p(n1+1:end,:),L_or_U);
            x1 = LUBF_sol(Factor.A11,p(1:n1,:)-BF_apply(Factor.A12,x2),L_or_U);
            y = [x1;x2];
        end
end
end
