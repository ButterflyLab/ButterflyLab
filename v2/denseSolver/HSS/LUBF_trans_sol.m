function y = LUBF_trans_sol(Factor, p,L_or_U)
% LUBF_trans_sol applies the data-sparse inverse of the transpose
...
switch L_or_U
    case 'U'
        if Factor.isLeaf
            ...
            x1 = LUBF_trans_sol(Factor.A11,p(1:n1,:),L_or_U);
            x2 = LUBF_trans_sol(Factor.A22,p((n1+1):end,:)-BF_trans_apply(Factor.A12,x1),L_or_U);
            y = [x1;x2];
        end
    case 'L'
        if Factor.isLeaf
            ...
            x2 = LUBF_trans_sol(Factor.A22,p(n1+1:end,:),L_or_U);
            x1 = LUBF_trans_sol(Factor.A11,p(1:n1,:)-BF_trans_apply(Factor.A21,x2),L_or_U);
            y = [x1;x2];
        end
end
end
